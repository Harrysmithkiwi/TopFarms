...
- [x] **Phase 23: Skills Taxonomy Consolidation + Admin Analytics** — Consolidate ~40 dairy-heavy skills into ~24 broad ag competencies across 6 categories; redesign `skills.sector` schema; migrate existing data; stand up admin skill-coverage analytics (IN SCOPE NOW) (completed 2026-05-30)
...
- [x] **Phase 28: Admin Dashboard UI/UX Rework** — Bring the admin surfaces (Daily Briefing, Lead Staging, Outreach, Leads, admin login) to a cohesive standard that conforms to the v2 design system; fix alignment / spacing / hierarchy / formatting; absorbs punch-list P-4 (rework), P-5 (display-name), P-8 (locality), P-9 (admin-login password show/hide), P-10 (matched-term on rows). Opened with a written design audit (GATE 1 taste approval, 28-AUDIT.md). P-11 (admin-door hardening) is future/optional, NOT in scope. Detail: `.planning/phases/28-admin-ui-ux-rework/`. **COMPLETE 2026-06-29** — P-4/P-5/P-8/P-10 (PR #7) + Phase 28b triage-power round T-1..T-5 (migrations 050-055) + P-9 admin-login password toggle (PR #25, submit-binding regression-tested). All five absorbed punch-list items closed; §7 partial-close cleared.
...
| 18.1. Pre-Launch Hardening | v2.0 | 7/7 | Partial-close 2026-05-10 (SC-1/3/4/5/6 PASS; SC-2 Stripe live-mode PEND-01 pending — Phase 21 plan 21-09 Task 2 DEFERRED to separate session 2026-05-18; SC-2 PASS flip gated on PEND-01 closure) | — |
...
- [x] 21-02-admin-doc-rpcs-PLAN.md — Wave 2: migration 033 — 4 SECURITY DEFINER admin doc RPCs (list_queue / approve / reject / request_more_info) + shape-contract tests GREEN (shipped 2026-05-17 commit 5cdc58c migration + 2026-05-18 commit 881202d test flip; Studio-applied; 3/3 Management-API verification PASS — see 21-02-SUMMARY.md)
- [x] 21-03-edge-fn-admin-bypass-PLAN.md — Wave 2: get-applicant-document-url admin role bypass branch + static-source regression guard preserving BFIX-05 gateway-trust (shipped 2026-05-17 commit 165b054 — 51/3 diff + 7/7 test GREEN; Edge fn deploy batched to plan 21-09 Task 1 operator batch — DEPLOYED 2026-05-18 with HTTP/2 200 admin-bypass smoke)
- [x] 21-04-auth-context-is-active-PLAN.md — Wave 3: AuthContext.loadRole single round-trip `select('role, is_active')` extension + ProtectedRoute /suspended redirect at correct guard position + IS-ACTIVE-* tests GREEN (shipped 2026-05-17)
- [x] 21-05-suspended-page-PLAN.md — Wave 3: SuspendedPage component at `src/pages/auth/Suspended.tsx` + /suspended route (outside ProtectedRoute) + 4/4 RTL tests GREEN (shipped 2026-05-17 commits e4fbfd0 + 3183e21)
- [x] 21-06-email-edge-fn-PLAN.md — Wave 4: send-document-status-email Deno Edge Function (3 templates: approved/rejected/needs_resubmission) + supabase/config.toml verify_jwt entry + static-source guard test (shipped 2026-05-17 commit bcde8d4; DEPLOYED 2026-05-18 with HTTP/2 200 send smoke confirming RESEND_API_KEY live in production)
- [x] 21-07-admin-documents-queue-PLAN.md — Wave 5: AdminDocumentsQueue page + main.tsx route + AdminSidebar "Documents" nav + email-side-effect RTL test; sibling X-Webhook-Secret removal in send-document-status-email (shipped 2026-05-18 commits f87dad8 + 2f580fc + d47a323; 9/9 admin-doc-queue tests GREEN + 10/10 send-document-status-email regression GREEN with flipped assertion)
- [x] 21-08-documents-verified-badge-PLAN.md — Wave 5: DocumentsVerifiedBadge component + ApplicantPanel employer-visible integration (Scenario B head-count query) + 5/5 RTL tests GREEN (shipped 2026-05-18 commit bb4bc82)
- [x] 21-09-track-a-milestone-close-PLAN.md — Wave 6: operator script — Edge Function deploys (Task 1 PASS 2026-05-18) + PEND-01 9-item checklist (Task 2 DEFERRED 2026-05-18 to separate session) + 5 visual smoke tests (Task 3 PASS 5/5 2026-05-18) + 5 Track B UATs (Task 4 PASS 5/5 2026-05-18) + 21-VERIFICATION.md + REQUIREMENTS.md ledger backfill + v2.0-MILESTONE-AUDIT update + atomic docs commit (Task 5 this commit). Task 6 (`/gsd:complete-milestone v2.0`) BLOCKED on PEND-01 — operator handoff for separate session.
