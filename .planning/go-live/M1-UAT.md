...
**Preview:** https://top-farms-7zjk4eotu-harrysymmanssmith-gmailcoms-projects.vercel.app
...
> **The preview URL moved on 2026-08-07** (was `top-farms-7huocqf7d`, then `9htj6k15e`). Six
> M4 landmark and heading fixes landed on `design/admin-gate` and `pricing/model-v3`
> afterwards, so the UAT now covers the tree that will actually merge: one `h1` and one
> `<main>` on `/jobs`, a named results region so desktop headings stop skipping `h1 → h3`,
> one `h1` on `/pricing`, the same nested-`<main>` defect on `/jobs/:id`, and a `<main>` for
> `/login` and `/signup`, which had none at all. Details, the corrections and the
> filed-not-fixed list in [`map.md`](map.md) § M4 *Landmark/heading closure*.
>
> **The 20/20 table below was re-run item by item against a preview of this tree and is still
...
> screenshot-checked. Against this exact preview the landmark guard and the anonymous axe
> sweep are **15/15**. Gates on the merged tree unchanged: `tsc -b` 0, vitest 644/0, lint 0
> errors at the 54 pin, design-gate 16 at the 17 pin, build OK.
>
> Nothing in the sections below changed — the fixes are semantic, so §1–§6 read the same. The
> one thing worth an extra glance while you are on `/jobs/:id` in §2: the job detail page's
> outer wrapper changed element, so if anything looks vertically off there, that is the
> suspect. Prod has no listings, so it could not be browser-checked before you post one.
