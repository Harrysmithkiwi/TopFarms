...
| After Phase 3 | ~72 | ~69 | **Item 1 NOT applied and that is the finding.** Bare `ALTER VIEW` proven on prod to raise 42501 for anon, emptying `/jobs`; 059 §3 is only partly live (policy absent, anon grants absent); even the full restore regresses seeker application history for filled jobs (`before=1 after=0`). Migration 104 records the posture; behaviour unchanged. Advisor ERROR still open BY DECISION — three options recorded, all change the security model. Item 2 (12 no-policy tables) not started. Scored +1, not +3. |
| After Phase 4 | ~75 | ~73 | `Sentry.setUser` (id only) at both session entry points + pendingUserId latch for the lazy chunk; `environment` now host-derived so preview cannot report as production; 14 seeker-path `console.error` -> `reportError` (grep: 0 remain under seeker dashboard/onboarding). 15 tests. **Not yet scored the full +3: no deliberate prod probe error has been confirmed arriving in Sentry with the id tag** — that needs the Sentry UI, operator-owned. |
