  ...
  return (
    <button
      ...
      className={cn(
        ...
        'flex flex-col gap-4',
        isSelected
          ? 'border-fern bg-[rgba(74,124,47,0.06)] shadow-sm'
          : 'border-fog bg-white hover:border-mid hover:shadow-sm',
      )}
    >
      {/* Popular badge */}
      {isPopular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span className="bg-fern text-white text-[11px] font-body font-semibold px-3 py-0.5 rounded-full">
            Best Value
          </span>
        </div>
      )}
      ...
      {/* Tier name */}
      <div>
        <h3 className="text-[15px] font-body font-bold text-ink">{name}</h3>
        ...
        <div className="mt-1 relative inline-block">
          ...
          {isFree && (
            <span className="ml-2 text-[13px] font-body font-semibold text-fern">
              Free!
            </span>
          )}
        </div>
        ...
        {isFree && (
          <p className="text-[11px] font-body text-moss mt-0.5">First listing free</p>
        )}
      </div>
          ...
          <li key={i} className="flex items-start gap-2">
            <Check
              className={cn(
                ...
                isSelected ? 'text-fern' : 'text-moss',
              )}
            />
            <span className="text-[12px] font-body text-mid leading-tight">{feature}</span>
          </li>
        ))}
      </ul>
    </button>
