...
| `031_rls_initplan_performance.sql` | **NOT IN REGISTRY** | — | 2026-05-10 (Phase 18.2-03) | Studio-applied. auth_rls_initplan sweep — all RLS policies rewritten from bare `auth.uid()` to `(select auth.uid())`. Performance-only, no behaviour change. Verified via pg_policies USING clause inspection — 34 compliant policies, 0 rows with bare auth.uid() remaining (see 18.2-03-SUMMARY.md). **Postgres normalization note:** Postgres normalises `(select auth.uid())` on storage to `( SELECT auth.uid() AS uid)` in pg_policies.qual. The correct verification LIKE pattern is `LIKE '% SELECT auth.uid() AS uid%'` — NOT `LIKE '%(select auth.uid())%'` which won't match the stored form. The original plan verification query used the wrong pattern; the corrected query was confirmed by the operator (34 compliant, 0 bare). |
